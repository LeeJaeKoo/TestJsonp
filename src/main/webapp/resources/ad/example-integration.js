 ...
 * 
 * For each content video, this plugin plays one preroll and one midroll. Ad
 * content is chosen randomly from the URLs listed in inventory.json.
 */
(function(window, document, vjs, undefined) {
	...
	/**
	 * Register the ad integration plugin. To initialize for a player, call
	 * player.exampleAds().
	 * 
	 * @param {mixed}
	 *            options Hash of obtions for the exampleAds plugin.
	 */
	registerPlugin(
			'exampleAds',
			function(options) {
				...
				adServerUrl = (options && options.adServerUrl)
						|| "inventory.json", midrollPoint = (options && options.midrollPoint) || 15, playPreroll = options
						&& options.playPreroll !== undefined ? options.playPreroll
						: true, playMidroll = options
						&& options.playMidroll !== undefined ? options.playMidroll
						: true, playPostroll = options
						&& options.playPostroll !== undefined ? options.playPostroll
						: true,
				...
				requestAds = function() {
					...
					// fetch ad inventory
					// the 'src' parameter is ignored by the example
					// inventory.json flat file,
					// but this shows how you might send player information
				...
				playAd = function() {
					...
					var media = state.inventory[Math.floor(Math.random()
				...
				// request ad inventory whenever the player gets new content to
				// play
